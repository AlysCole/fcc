const Grid = {
  ...
  getRandomPointWithin: (x1, x2, y1, y2) => {
    return {
      x: Math.randomBetween(x1, x2),
      y: Math.randomBetween(y1, y2)
    };
  },
  getRandomMatchingCellWithin: (x1, x2, y1, y2, type, grid) => {
    let cell = {
      x: Math.randomBetween(x1, x2),
      y: Math.randomBetween(y1, y2)
    };
  ...
  calculateApproxDistance: (x1, y1, x2, y2) => {
    return Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2));
  },
  determinePath: (startX, startY, targetX, targetY, grid) => {
    let closed = [],
      open = [];

    if (startX == targetX && startY == targetY) return [];

    const getCellFromList = (x, y, list) => {
      ...
      adjacentCells = adjacentCells.map(cell => {
        ...
        pathObj.g +=
          ...
          cell.direction == "w"
            ...
            : 15;
        pathObj.h =
          Grid.calculateApproxDistance(cell.x, cell.y, targetX, targetY) * 10;
